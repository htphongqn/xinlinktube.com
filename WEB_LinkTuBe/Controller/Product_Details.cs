...
namespace Controller
{
    public class Product_Details
    {
        ...
        #endregion
        public string Load_Hang(int cat_id)
        {
            try
            {
        ...
        public void Adddie(string _news_seo)
        {
            var list = db.ESHOP_NEWs.Where(n => n.NEWS_SEO_URL == _news_seo).ToList();
            if (list.Count > 0)
            {
                if (list[0].UNIT_ID2 == null)
                    list[0].UNIT_ID2 = 1;
                else
                    list[0].UNIT_ID2++;
                db.SubmitChanges();
