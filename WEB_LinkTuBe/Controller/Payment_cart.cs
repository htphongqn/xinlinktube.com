...
namespace Controller
{
    public class Payment_cart
    {
        ...
        //Inser order
        private int Insert_Order(decimal Total_All, string Name, string OrderCode, string Email, string Address, int PaymentID, Guid News_guid, string Desc, DateTime Date, string Phone, int userid)
        {
            int _orderID = -1;
        ...
        //Check cart
       public List<ESHOP_ORDER> GetDonHangtheoCutomerID(string userid)
       {
           try
           {
               var list = db.ESHOP_ORDERs.Where(n => n.CUSTOMER_ID.ToString() == userid).OrderByDescending(n => n.ORDER_ID).ToList();
               return list;
           }
           catch
           {
       ...
       public IQueryable<ESHOP_ORDER_ITEM> GetItemtheoOderid(string Order_ID)
       {
           try
           {
               var list = db.ESHOP_ORDER_ITEMs.Where(n => n.ORDER_ID.ToString() == Order_ID).OrderByDescending(n => n.ITEM_ID);
               return list;
           }
           catch
           {
       ...
       public decimal Get_SumMoney(object oder_id)
       {
           try
           {
               var list = db.ESHOP_ORDERs.Where(n => n.ORDER_ID == Utils.CIntDef(oder_id, 0)).ToList();
               if (list.Count() > 0)
               {
                   return Utils.CDecDef(list[0].ORDER_TOTAL_AMOUNT);
               }
               return 0;
           }
           catch
       ...
       public string GET_TITLE(string news_id)
       {
           try
           {
               var list = db.ESHOP_NEWs.Where(n => n.NEWS_ID.ToString() == news_id).ToList();
               if (list.Count() > 0)
               {
       ...
       public string GET_IMG_PAY(string news_id)
       {
           try
           {
               var list = db.ESHOP_NEWs.Where(n => n.NEWS_ID.ToString() == news_id).ToList();
               if (list.Count() > 0)
               {
                   return GetImageT_News(news_id, list[0].NEWS_IMAGE3);
               }
               return "";
           }
           catch
