...
namespace Controller
{
    public class News_details
    {
        ...
        public string Showfilehtm(int _newsID)
        {
            try
            {
        ...
        public List<News_details_entity> Load_othernews_moihon(int _newid)
        {
            try
            {
                List<News_details_entity> l = new List<News_details_entity>();
                if (_newid != 0)
                {
                    ...
                    if (s.Count > 0)
                    {
                        var _tinTucKhac = (from c in db.ESHOP_NEWS_CATs
                                           ...
                                           join b in db.ESHOP_CATEGORies on c.CAT_ID equals b.CAT_ID
                                           where a.NEWS_ID > _newid && b.CAT_ID == s[0].CAT_ID
                                           select new {a.NEWS_VIDEO_URL, a.NEWS_FIELD2, a.NEWS_FIELD3, a.NEWS_FIELD1, a.NEWS_FIELD4, a.NEWS_PRICE2, a.NEWS_ID, a.NEWS_TITLE, a.NEWS_IMAGE3, a.NEWS_PRICE1, a.NEWS_DESC, a.NEWS_SEO_URL, a.NEWS_URL, a.NEWS_ORDER, a.NEWS_ORDER_PERIOD, a.NEWS_PUBLISHDATE }).Distinct().OrderBy(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_ORDER).Take(5).Distinct();
                        if (_tinTucKhac.ToList().Count > 0)
                        {
                            foreach (var i in _tinTucKhac)
                            {
       ...
       //Insert comment news detail
        ...
        //Gallery
        public List<ESHOP_NEWS_IMAGE> listimg(string new_seo_url)
        {
            var list = (from a in db.ESHOP_NEWs
                        join b in db.ESHOP_NEWS_IMAGEs on a.NEWS_ID equals b.NEWS_ID
                        where a.NEWS_SEO_URL == new_seo_url
                        select b).ToList();
            if (list.Count() <= 0)
            {
                var list1 = (from a in db.ESHOP_CATEGORies
                             join b in db.ESHOP_NEWS_CATs on a.CAT_ID equals b.CAT_ID
                             join c in db.ESHOP_NEWs on b.NEWS_ID equals c.NEWS_ID
                             where a.CAT_SEO_URL == new_seo_url
                             select c).ToList();
                if (list1.Count() > 0)
                {
                    list = (from a in db.ESHOP_NEWs
                            join b in db.ESHOP_NEWS_IMAGEs on a.NEWS_ID equals b.NEWS_ID
                            where a.NEWS_SEO_URL == list1[0].NEWS_SEO_URL
                            select b).ToList();
                }
            }
            return list;
        ...
        public class Entity_title
        {
            ...
            public string New_update { get; set; }
        }
