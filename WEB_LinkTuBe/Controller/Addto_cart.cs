...
namespace Controller
{
    public class Addto_cart
    {
        ...
        #endregion
        public string Get_New_Seo_URL(object id)
        {
            var list = db.ESHOP_NEWs.Where(n => n.NEWS_ID == Utils.CIntDef(id,0)).ToList();
            if (list.Count() > 0)
                return "/" + list[0].NEWS_SEO_URL + ".html";
            return "";
        }
        public int Add_To_Cart(int News_id, object Guid, int quantity)
        {
                decimal _dPrice = 0;
                ...
                {
                    //if (Utils.CIntDef(_news.ToList()[0].NEWS_FIELD3) <= 0)
                    //    return 1;
                ...
                if (_dPrice != 0)
                {
                    if (Check_Exist_Item(News_id, (Guid)Guid))
                    {
                        ...
                        //var _vBasket = db.GetTable<ESHOP_BASKET>();
                        //db.ESHOP_BASKETs.DeleteAllOnSubmit(_vBasket);
                        //db.SubmitChanges();
                        var _vBasket = db.GetTable<ESHOP_BASKET>().Where(a => a.NEWS_ID == News_id);
                        if (_vBasket.ToList().Count > 0)
                        {
                            _vBasket.FirstOrDefault().BASKET_QUANTITY += Utils.CIntDef(quantity) != 0 ? quantity : 1;
                            db.SubmitChanges();
                        }
                    }
                    else
