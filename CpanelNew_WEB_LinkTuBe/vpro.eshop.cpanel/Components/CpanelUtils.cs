...
namespace vpro.eshop.cpanel.Components
{
    public static class CpanelUtils
    {
        ...
        public static List<string[]> listAds()
        {
            ...
            { 
                new string[] { "0", "Show chính" }, 
              
                new string[] { "2", "Ads trượt trái" },
                new string[] { "3", "Ads trượt phải" },
        ...
        public static void TransformTableWithSpace2(ref DataTable source, DataTable dest, DataRelation rel, DataRow parentRow)
        {
            if (parentRow == null)
            {
                foreach (DataRow row in source.Rows)
                {
                    if (!row.HasErrors && (Utils.CIntDef(row["MENU_PARENT_ID"]) <= 0))
                    {
                        row["MENU_NAME"] = (Utils.CIntDef(row["MENU_RANK"]) <= 1 ? "" : Duplicate("------", Utils.CIntDef(row["MENU_RANK"]))) + row["MENU_NAME"];
                        dest.Rows.Add(row.ItemArray);
                        row.RowError = "dirty";
                        if (Utils.CStrDef(row["MENU_NAME"]) != "------- Root -------")
                            TransformTableWithSpace2(ref source, dest, rel, row);
                    }
                }
            }
            else
            {
                ...
                if (!parentRow.HasErrors)
                {
                    parentRow["MENU_NAME"] = (Utils.CIntDef(parentRow["MENU_RANK"]) <= 1 ? "" : Duplicate("------", Utils.CIntDef(parentRow["MENU_RANK"]))) + parentRow["MENU_NAME"];
                    dest.Rows.Add(parentRow.ItemArray);
                    parentRow.RowError = "dirty";
                }
                if (children != null && children.Length > 0)
                {
                    foreach (DataRow child in children)
                    {
                        if (!child.HasErrors)
                        {
                            child["MENU_NAME"] = (Utils.CIntDef(child["MENU_RANK"]) <= 1 ? "" : Duplicate("------", Utils.CIntDef(child["MENU_RANK"]))) + child["MENU_NAME"];
                            dest.Rows.Add(child.ItemArray);
                            child.RowError = "dirty";
                            TransformTableWithSpace2(ref source, dest, rel, child);
                        }
                    }
                }
            }
