...
namespace vpro.eshop.cpanel.page
{
    public partial class category : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Hyperback.NavigateUrl = "category_list.aspx";
            if (m_cat_id == 0)
            {
            ...
            else Hyperseo_cate.HRef = "category-seo-editor.aspx?id=" + m_cat_id;
        ...
        protected void lbtSave_Click(object sender, EventArgs e)
        {
            ...
            string seo_url = txtSeoUrl.Value.Trim();
            if (CheckExitsLink(seo_url) || CheckExitsLink_News(seo_url))
            {
                txtSeoUrl.Value = txtSeoUrl.Value + "-" + "C" + loadmaxid();
                SaveInfo();
            }
            else
                SaveInfo();
        ...
        public List<string[]> getNamePos()
        {
            ...
            foreach (ListItem l in rblPos.Items)
            {
                string[] a = new string[] { l.Value, l.Text };
                list.Add(a);
            }
            return list;
        }
        ...
        private void getInfo()
        {
            try
            {
                ...
                if (G_info.Count > 0)
                {
                    ddlCategory.SelectedValue = Utils.CStrDef(G_info[0].CAT_PARENT_ID);
                    txtCode.Value = G_info[0].CAT_CODE;
                    txtName.Value = G_info[0].CAT_NAME;
                    txtDesc.Value = G_info[0].CAT_DESC;
                    txtUrl.Value = G_info[0].CAT_URL;
                    ddlTarget.SelectedValue = G_info[0].CAT_TARGET;
                    rblStatus.SelectedValue = Utils.CStrDef(G_info[0].CAT_STATUS);
                    //rblAccess.SelectedValue = Utils.CStrDef(G_info[0].CAT_ACCESS);
                    rblPos.SelectedValue = Utils.CStrDef(G_info[0].CAT_POSITION);
                    rblShowFooter.SelectedValue = Utils.CStrDef(G_info[0].CAT_SHOWFOOTER);
                    rblShowItems.SelectedValue = Utils.CStrDef(G_info[0].CAT_SHOWITEM);
                    rblCatPeriod.SelectedValue = Utils.CStrDef(G_info[0].CAT_PERIOD);
                    rblLanguage.SelectedValue = Utils.CStrDef(G_info[0].CAT_LANGUAGE);
                    txtOrder.Value = Utils.CStrDef(G_info[0].CAT_ORDER);
                    txtOrderPeriod.Value = Utils.CStrDef(G_info[0].CAT_PERIOD_ORDER);
                    txtpageItem.Value = Utils.CStrDef(G_info[0].CAT_PAGEITEM);
                    //lblCount.Text = Utils.CStrDef(G_info[0].CAT_COUNT);
                    rblCatType.SelectedValue = Utils.CStrDef(G_info[0].CAT_TYPE);

                    //seo
                    txtSeoTitle.Value = Utils.CStrDef(G_info[0].CAT_SEO_TITLE);
                    txtSeoKeyword.Value = Utils.CStrDef(G_info[0].CAT_SEO_KEYWORD);
                    txtSeoDescription.Value = Utils.CStrDef(G_info[0].CAT_SEO_DESC);
                    txtSeoUrl.Value = Utils.CStrDef(G_info[0].CAT_SEO_URL);

                    //english language
                    //txtNameEn.Value = G_info[0].CAT_NAME_EN;
                    //txtCodeEn.Value = G_info[0].CAT_CODE_EN;
                    //txtDescEn.Value = G_info[0].CAT_DESC_EN;

                    //txtSeoTitleEn.Value = Utils.CStrDef(G_info[0].CAT_SEO_TITLE_EN);
                    //txtSeoKeywordEn.Value = Utils.CStrDef(G_info[0].CAT_SEO_KEYWORD_EN);
                    //txtSeoDescriptionEn.Value = Utils.CStrDef(G_info[0].CAT_SEO_DESC_EN);
                    //txtSeoUrlEn.Value = Utils.CStrDef(G_info[0].CAT_SEO_URL_EN);


                    //image 1
                    if (!string.IsNullOrEmpty(G_info[0].CAT_IMAGE1))
                    {
                        trUploadImage1.Visible = false;
                        trImage1.Visible = true;
                        Image1.Src = PathFiles.GetPathCategory(m_cat_id) + G_info[0].CAT_IMAGE1;
                        hplImage1.NavigateUrl = PathFiles.GetPathCategory(m_cat_id) + G_info[0].CAT_IMAGE1;
                        hplImage1.Text = G_info[0].CAT_IMAGE1;
                    }
                    else
                    {
                    ...
                    ////image 2
                    //if (!string.IsNullOrEmpty(G_info[0].CAT_IMAGE2))
                    //{
                    //    trUploadImage2.Visible = false;
                    //    trImage2.Visible = true;
                    //    Image2.Src = PathFiles.GetPathCategory(m_cat_id) + G_info[0].CAT_IMAGE2;
                    //    hplImage2.NavigateUrl = PathFiles.GetPathCategory(m_cat_id) + G_info[0].CAT_IMAGE2;
                    //    hplImage2.Text = G_info[0].CAT_IMAGE2;
                    //}
                    //else
                    //{
                    ...
                    ////image 3
                    //if (!string.IsNullOrEmpty(G_info[0].CAT_IMAGE3))
                    //{
                    //    trUploadImage3.Visible = false;
                    //    trImage3.Visible = true;
                    //    Image3.Src = PathFiles.GetPathCategory(m_cat_id) + G_info[0].CAT_IMAGE3;
                    //    hplImage3.NavigateUrl = PathFiles.GetPathCategory(m_cat_id) + G_info[0].CAT_IMAGE3;
                    //    hplImage3.Text = G_info[0].CAT_IMAGE3;
                    //}
                    //else
                    //{
        ...
        private bool CheckExitsLink_News(string strLink)
        {
            try
            {
        ...
        private int loadmaxid()
        {
            try
            {
                var exits = (from c in DB.ESHOP_CATEGORies select c).OrderByDescending(n => n.CAT_ID).ToList();

                if (exits.Count() > 0)
                    return Utils.CIntDef(exits[0].CAT_ID) + 1;
